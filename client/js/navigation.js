...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Inject Enterprise Footer
  const footerHTML = `
    <footer class="enterprise-footer">
      <div class="footer-grid">
        <div class="footer-col brand-col">
          <div class="footer-logo">
            <img src="logo.png" alt="Resumetrices Logo" style="height: 32px; width: 32px; object-fit: contain;">
        ...
        <div class="footer-col">
          ...
          <ul>
            ...
            <li><a href="#" class="disabled-link">Cookie Policy</a></li>
            <li><a href="#" class="disabled-link">Data Security</a></li>
          </ul>
        </div>
      </div>
  ...
  function injectFooter() {
    const existingFooters = document.querySelectorAll('.master-footnote-footer');
    if (existingFooters.length > 0) {
      existingFooters.forEach(f => {
        const temp = document.createElement('div');
        temp.innerHTML = footerHTML;
        f.replaceWith(temp.firstElementChild);
      });
    } else {
      ...
      if (mainContainer) {
        const temp = document.createElement('div');
        temp.innerHTML = footerHTML;
        if (!mainContainer.querySelector('.enterprise-footer')) {
          const pageView = mainContainer.querySelector('.page-view') || mainContainer;
          pageView.appendChild(temp.firstElementChild);
        }
      }
    }
  }
